...
public class FlowSilence implements CommandExecutor{

    private chatflow plugin;

    public FlowSilence(chatflow plugin){
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args){
        if (args.length == 1)
        {
            if (chatflow.hasPermissions(sender, "Chatflow.silence"))
            {
                ...
                plugin.silencedPlayers.add(playername.toLowerCase());
                plugin.toggledPlayers.remove(playername.toLowerCase());
                plugin.silencedPlayers.save();
                sender.sendMessage(
                        ChatColor.YELLOW + "Successfully silenced player " + ChatColor.GOLD + playername);
            } else
            {
                sender.sendMessage(
        ...
        {
            sender.sendMessage(ChatColor.YELLOW + "Correct usage is /flowsilence <player>");
        }
        return true;
    }
